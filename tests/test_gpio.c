...
void TEST_GPIO_INIT(void)
{
    ...
    HAL_GPIO_Init();
    // Assert
    ASSERT_EQ(DDRD, 0b00000000); // port dir INPUT
    ASSERT_EQ(PORTD, 0b11111111); // port pull UP
}
...
void TEST_GPIO_CONFIGPIN(void)
{
    ...
    // Act
    // Assert
    ASSERT_EQ(DDRD, 0b00000001); // port dir OUTPUT
    ASSERT_EQ(PORTD, 0b11111110); // port pull DOWN
}
...
void TEST_GPIO_WRITEPIN(void)
{
    ...
    // Assert 1
    ASSERT_EQ(PORTD, 0b11111010);
    // Act 2
    HAL_GPIO_WritePin(PIN_2, GPIO_STATE_HIGH);
    // Assert 2
    ASSERT_EQ(PORTD, 0b11111110);
}
